...
inline LatticeFasterDecoder::Token *LatticeFasterDecoder::FindOrAddToken(
  ...
  if (e_found == NULL) {  // no such token presently.
    ...
    if (tok->tot_cost > tot_cost) {  // replace old token
      ...
      if (tok->tot_cost != tok->tot_cost) {
	std::cout << "A.  Setting tok->tot_cost to NaN\n";
      }
...
void LatticeFasterDecoder::PruneForwardLinks(
  ...
  *links_pruned = false;
  ...
  while (changed) {
	...
	if (link_extra_cost != link_extra_cost) {
	  std::cout << "link_extra_cost is NaN. Here is some debug info:\n";
	  std::cout << "\tnext_tok->extra_cost = " << next_tok->extra_cost << std::endl;
	  std::cout << "\ttok->tot_cost = " << tok->tot_cost << std::endl;
	  std::cout << "\tlink->acoustic_cost = " << link->acoustic_cost << std::endl;
	  std::cout << "\tlink->graph_cost = " << link->graph_cost << std::endl;
	  std::cout << "\tnext_tok->tot_cost = " << next_tok->tot_cost << std::endl;
	}
      ...
      if (tok->extra_cost != tok->extra_cost) {
	std::cout << "B. Setting tok->extra_cost to NaN\n";
      }
...
void LatticeFasterDecoder::ProcessEmitting(DecodableInterface *decodable) {
  ...
  for (Elem *e = final_toks, *e_tail; e != NULL; e = e_tail) {
    ...
    if (tok->tot_cost <=  cur_cutoff) {
	  ...
	  if (ac_cost != ac_cost) {
	    std::cout << "ac_cost is NaN at A" << std::endl;
	    std::cout << "\tcost_offset = " << cost_offset << std::endl;
	    std::cout << "\tdecodable->LogLikelihood(" << frame << ", " << arc.ilabel << ") = " << decodable->LogLikelihood(frame, arc.ilabel) << std::endl;
	  }
